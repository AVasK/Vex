...
int main(int argc, char* argv[]) {
    ...
    for (int i=0; i<n_tests; i++)
    {
        auto t = timing::nsTimer("APPLE BLAS SAXPY");
        cblas_saxpy(N, 7.0, blas_w.data(), 1, blas_v.data(), 1);
        //cblas_saxpy(N, 1.0, blas_z.data(), 1, blas_v.data(), 1);
        //cblas_saxpy(N, 1.0, blas_x.data(), 1, blas_v.data(), 1);
    }
    ...
    for (int i=0; i<n_tests; i++)
    {
        auto t = timing::nsTimer("openBLAS SAXPY");
    ...
    for (int i=0; i<n_tests; i++)
    {
        auto t = timing::nsTimer("openBLAS SAXPY (no asm)");
    ...
    for (int i=0; i<n_tests; i++)
    {
        auto t = timing::nsTimer("std::vector");
        //res = std::vector<T>(N);
        for (size_t i=0; i<vv.size(); ++i) {
            //vv[i] += vw[i];
            //vv[i] += 7;
    ...
    for (int i=0; i<n_tests; i++)
    {
        auto t = timing::nsTimer("std::valarray");
        va += wa*7;
        //va += wa + za + xa;
